            ...
            <section className="space-y-4">
              <div className="space-y-2">
                <div className="flex items-center gap-4">
                  <p className={`text-xs ${mutedText}`}>// {detailMeta?.date ?? samplePostContent.date} - {detailMeta?.readTime ?? samplePostContent.readTime}</p>
                  <span className={`text-xs ${isDarkMode ? "text-[#D4FF00]" : "text-[#3F5200]"}`}>
                    [v: {(post?.views ?? samplePostContent.views).toLocaleString()}]
                  </span>
                </div>
                        <h1 className="text-xl">{post?.title ?? samplePostContent.title}</h1>
              ...
              {post ? (
                ...
                <article className="prose-terminal space-y-4 text-sm leading-relaxed font-[var(--font-jetbrains-mono),monospace]">
                  ...
                  .map((block, index) => {
                    ...
                    return <p key={index} className={mutedText} dangerouslySetInnerHTML={{ __html: formatted }} />
                  })}
                </article>
              )}
