...
import { V0DetailContent, hasRenderableDetailContent } from "@/components/v0/public/detail-content"
...
export function DetailProjectScreen({
  ...
  const renderedResources = post ? collectBlockDocumentResources(post.content) : { linkUrls: [], assetIds: [], assetUrls: [] }
  const visibleLinks = post
    ? post.links.filter((link) => !renderedResources.linkUrls.includes(normalizeContentResourceUrl(link.normalizedUrl ?? link.url)))
    : []
          ...
          <div className="w-full max-w-2xl space-y-6">
            ...
            <section className="space-y-6">
              <div className="space-y-3">
                ...
                <h1 className="text-xl">{post?.title ?? sampleProjectContent.title}</h1>
                <p className={`text-sm ${mutedText} leading-relaxed`}>{post?.excerpt ?? sampleProjectContent.description}</p>
              </div>
              ...
              {post?.tags.length ? (
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {post.tags.map((tag) => (
                      <span key={tag} className={`text-xs px-2 py-1 border ${borderColor}`}>
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {sampleProjectContent.techStack.map((tech) => (
                      <span key={tech} className={`text-xs px-2 py-1 border ${borderColor}`}>
                        {tech}
                      </span>
                    ))}
                  </div>
              ...
              {(post?.links.length ? visibleLinks.length > 0 : sampleProjectContent.urls.length > 0) ? (
                <div className="space-y-3">
                  ...
                  <div className="space-y-3">
                    {(post?.links.length
                      ? visibleLinks.map((link) => ({
                        ...
                        domain: link.siteName ?? link.normalizedUrl ?? link.url,
              ...
              <div className={`pt-8 border-t ${borderColor} space-y-4`}>
                ...
                <div className="flex flex-wrap gap-4 text-xs">
                  <button type="button" onClick={() => void handleShare()} className={`${hoverBg} px-2 py-1`}>
                    {actionState === "shared" ? "[shared]" : "[share]"}
                  </button>
                  <button type="button" onClick={() => void handleCopyLink()} className={`${hoverBg} px-2 py-1`}>
